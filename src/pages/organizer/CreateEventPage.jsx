...
function StepBasicInfo({ form, setForm, categories, fieldErrors }) {
...
// ── Ticket type row ───────────────────────────────────────────
function TicketTypeRow({ tt, index, onChange, onRemove }) {
  return (
...
// ── Step 2: Tickets ───────────────────────────────────────────
function StepTickets({ form, setForm }) {
...
// ── Step 3: Publish ───────────────────────────────────────────
function StepPublish({ form, setForm, error }) {
  function set(field, value) {
    setForm((prev) => ({ ...prev, [field]: value }));
  }
      ...
      <div className="flex flex-col gap-3">
        ...
        {[
          ...
          <button
            ...
            className={`w-full text-left flex items-start gap-4 p-4 rounded-card border-2 transition-all duration-150 ${
  ...
  return (
    <div className="flex flex-col min-h-screen bg-main-bg">
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-6 py-8">
        ...
        {/* Step content card */}
        <div className="bg-card border border-border rounded-card p-6 sm:p-8 mb-6">
          {currentStep === 1 && (
            <StepBasicInfo
              form={form}
              setForm={setForm}
          ...
          {currentStep === 2 && <StepTickets form={form} setForm={setForm} />}
          {currentStep === 3 && (
